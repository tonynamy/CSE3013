...
void InitTetris(){
	...
	if(mRecFlag) {
		...
		mRecRoot->level = 1;
		mRecRoot->accumulatedScore = 0;

		int* compressed = compressField(field);
		memcpy(mRecRoot->recField, compressed, sizeof(mRecRoot->recField));
		free(compressed);
...
void play(){
	...
	do{
		...
		if(command==QUIT){
			...
			refresh();
			getch();
			if(menuFlag==MENU_PLAY) newRank(score);
			return;
		}
	}while(!gameOver);
	if(mRecFlag) {
...
int recommend(RecNode *root){
	...
	int nextBlockNum = nextBlock[root->level-1];
	...
	for(int r=0;r<NUM_OF_ROTATE;r++) {
		...
		for(int x=-BLOCK_WIDTH;x<=WIDTH+BLOCK_WIDTH;x++) {
			...
			while(CheckToMove(root->recField, nextBlockNum, r, ++y, x));
			y--;
			if(y<0) continue;
			...
			child->accumulatedScore = 0;
			child->accumulatedScore += AddBlockToField(child->recField, nextBlockNum, r, y, x);
...
int* compressField(char f[HEIGHT][WIDTH]) {

	int* compressed = (int*) malloc(sizeof(int)*WIDTH);
...
int modified_recommend(MRecNode *root) {
	...
	int rotatePruningNum = NUM_OF_ROTATE;

	int nextBlockNum = nextBlock[root->level-1];

	if(nextBlockNum == 0) rotatePruningNum=1;
	else if (nextBlockNum == 4||nextBlockNum==5||nextBlockNum==6) rotatePruningNum=2;

	// user code
	for(int r=0;r<NUM_OF_ROTATE;r++) {

		if(r>=rotatePruningNum) continue;

		for(int x=-BLOCK_WIDTH;x<=WIDTH+BLOCK_WIDTH;x++) {
			
			int y=-1;

			char rDecompressed[HEIGHT][WIDTH] = {0};

			for(int j=0;j<WIDTH;j++) {
				for(int i=root->recField[j];i<HEIGHT;i++) {
					rDecompressed[i][j] = 1;
				}
			}

			while(CheckToMove(rDecompressed, nextBlockNum, r, ++y, x));
			y--;
			if(y<0) continue;

			MRecNode* child = (MRecNode*) malloc(sizeof(MRecNode));
			root->child[i++] = child;
			...
			char decompressed[HEIGHT][WIDTH] = {0};
			for(int j=0;j<WIDTH;j++) {
				for(int i=child->recField[j];i<HEIGHT;i++) {
					rDecompressed[i][j] = 1;
				}
			}

			child->accumulatedScore += AddBlockToField(decompressed, nextBlockNum, r, y, x);
			child->accumulatedScore += DeleteLine(decompressed);

			int* compressed = compressField(decompressed);

			memcpy(child->recField, compressed, sizeof(child->recField));
			...
			if(root->level < BLOCK_NUM) {
				child->accumulatedScore += modified_recommend(child);
			}
